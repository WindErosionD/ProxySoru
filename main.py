...
if colorlog and hasattr(colorlog, "ConsoleHandler"):
...
if (__name__ == "__main__"):
	...
	logger.info("ProxySoru SSRSpeed v{}".format(config["VERSION"]))
	...
	_cfg_method = str(config.get("method", "") or "").strip().upper()
	_cfg_method_map = {
		"SOCKET": "SOCKET",
		"ST_ASYNC": "ST_ASYNC",
		"SPEEDTESTNET": "SPEED_TEST_NET",
		"SPEED_TEST_NET": "SPEED_TEST_NET",
		"FAST": "FAST",
		"YOUTUBE": "YOUTUBE",
		"NETFLIX": "NETFLIX",
	}
	if _cfg_method in _cfg_method_map:
		TEST_METHOD = _cfg_method_map[_cfg_method]
		logger.info("Test method from ssrspeed_config.json: %s", TEST_METHOD)
	...
	if (options.import_file and CONFIG_LOAD_MODE == 0):
		...
		sc.sortMethod = SORT_METHOD or "SPEED"
	...
	sc.start_test(options.topology)
