...
class SpeedTestMethods(object):
	...
	def startTest(self,method = "ST_ASYNC"):
		...
		if (method == "SPEED_TEST_NET"):
			...
			try:
				# METHOD 仅用于在「SOCKET 测速」下区分 YouTube / Netflix 专用流；其余一律走经典 socket 下载测速。
				if METHOD == "YOUTUBE":
					return stYtb.speedTestYtb(LOCAL_PORT)
				if METHOD == "NETFLIX":
					return stNF.speedTestNetflix(LOCAL_PORT)
				result = stSocket.speedTestSocket(LOCAL_PORT)
				# SOCKET 仅 HTTP:80，CDN 常只返回极少字节；自动改用 ST_ASYNC（HTTPS+SOCKS）
				if config.get("speed", True) and (result[3] or 0) < 64 * 1024:
					logger.warning(
						"SOCKET download too small (%d bytes), retry with ST_ASYNC (HTTPS).",
						result[3] or 0,
					)
					self.__initSocket()
