...
class ParserClash:
	...
	def __convert_trojan_cfg(self, cfg):
		...
		return {
			"run_type": "client",
			"local_addr": ssr_config["localAddress"],
			"local_port": int(ssr_config["localPort"]),
			"remote_addr": server,
			"remote_port": port,
			"password": [
				password
	...
	def parse_config(self, clash_cfg):
		try:
			...
			logger.error("Clash config parse failed: invalid YAML content.")
			return
		if not isinstance(clash_cfg, dict):
			logger.error("Clash config parse failed: root node is not a mapping.")
			return
		proxies = clash_cfg.get("proxies", [])
		if not isinstance(proxies, list):
			logger.error("Clash config parse failed: 'proxies' is not a list.")
			return
		for cfg in proxies:
